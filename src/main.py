...
from .scraper import FDAAdverseEventsScraper
...
async def main() -> None:
    async with Actor:
        ...
        Actor.log.info(
            "Starting FDA Adverse Events Scraper | mode=%s | max_results=%s",
            config.mode.value,
            max_results,
        )
        ...
        await Actor.set_status_message("Connecting to openFDA API...")
            ...
            try:
                async for item in scraper.scrape():
                    ...
                    if len(batch) >= batch_size:
                        ...
                        batch = []
                        await Actor.set_status_message(f"Scraped {count}/{max_results} adverse events")
        ...
        msg = f"Done. Scraped {count} adverse events."
        if state["failed"] > 0:
